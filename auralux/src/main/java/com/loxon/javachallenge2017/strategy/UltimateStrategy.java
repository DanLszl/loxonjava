...
public class UltimateStrategy extends Strategy {
    ...
    public Map<Integer, Double> calculateMagicNumbers(GameState gameState) {
        ...
        double w_attackConcentration = 1.0;
        double w_armiesStrengthQuotient = 1.0;
        double w_radii = 1.0;
        double w_possession = 1.0;
                ...
                .collect(
                        Collectors.toMap(
                                ...
                                entry -> {
                                    ...
                                    return w_attackConcentration        * entry.getValue()
                                            + w_armiesStrengthQuotient  * planetArmiesValues.get(id)
                                            + w_radii                   * planetRadiiValues.get(id)
                                            + w_possession              * possessionValues.get(id);
                                }
                        )
                );
